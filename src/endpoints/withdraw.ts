import { Data, Lucid, Script, TxHash, UTxO } from "@anastasia-labs/lucid-cardano-fork";
...
export async function txWithdrawNft(marketplaceAddr: string, redeemer: string, 
                    script : Script ,lucid : Lucid): Promise<TxHash> {
        ...
        if (!f_utxos) throw new Error ("Utxos not found");
...
const txHash = await signedTx.submit();
  ...
  function findSellerAddress (u : UTxO, addr : string, lucid : Lucid ) {
